...
import { UseCase, USE_CASES } from '../constants';
      ...
      <div className="max-w-6xl mx-auto px-6 py-12 sm:py-16">
        ...
        <div className="grid md:grid-cols-2 gap-8 items-center">
          <motion.div initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
            <img src="https://avatars.githubusercontent.com/u/1017329?v=4" alt="Gazi Pollob Hussain" className="w-40 h-40 rounded-full object-cover shadow-xl border-4 border-white dark:border-gray-700 mx-auto md:mx-0" onError={(e)=>{e.currentTarget.src='https://via.placeholder.com/160'}} />
          ...
          <motion.div initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} className="bg-white/60 dark:bg-gray-800/60 backdrop-blur-xl rounded-2xl shadow-lg border border-white/20 dark:border-white/10 p-6">
            <h3 className="text-teal-700 dark:text-teal-400 text-xl font-semibold">Philosophy Snapshot</h3>
            <p className="text-gray-700 dark:text-gray-300 mt-3">“AI should not replace life—it should help it flourish.” This guiding sentence is the heartbeat of AikoInfinity, informing our manifesto and product design.</p>

            <div className="mt-5 grid grid-cols-3 gap-3">
              {PILLARS.map((p) => (
                <div key={p.id} className="text-center p-3 rounded-lg bg-gradient-to-br from-white to-gray-50 dark:from-gray-700/50 dark:to-gray-800/50 border border-gray-200/50 dark:border-gray-700/50">
                  <div className={`w-10 h-10 rounded-full mx-auto mb-2 bg-gradient-to-br ${p.color} shadow-inner`} />
                  <div className="text-sm font-semibold text-gray-800 dark:text-gray-200">{p.title.split(" — ")[0]}</div>
                  <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">{p.desc.split(" ").slice(0,8).join(" ")}...</div>
                </div>
              ))}
            </div>
          </motion.div>
        ...
        {/* Pillars interactive */}
        <motion.div id="pillars" variants={pillVariants} initial="hidden" animate="enter" className="mt-16 grid md:grid-cols-3 gap-6">
          {PILLARS.map((p) => (
            <motion.div key={p.id} variants={pillVariants} whileHover={{ scale: 1.02 }} className="bg-white/60 dark:bg-gray-800/60 backdrop-blur-xl p-6 rounded-2xl shadow-lg border border-white/20 dark:border-white/10 hover:shadow-xl cursor-pointer">
              ...
              {activePillar === p.id && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="mt-4 text-sm text-gray-700 dark:text-gray-300">
